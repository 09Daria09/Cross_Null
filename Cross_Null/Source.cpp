...
HICON hIcon;            // иконка 
HWND hBut[10];          // кнопки (поле), 10я - таймер
...
static int t = 0;       // для таймера
static int t2 = 0;      // для таймера
static int si_zero = 0; // побед ноликов
static int si_cross = 0;// побед крестиков
static int si_draw = 0; // ничьих
...
BOOL CALLBACK DlgProc(HWND hWnd, UINT message, WPARAM wp, LPARAM lp)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        for (int i = 0; i < 9; i++)
        {
            hBut[i] = GetDlgItem(hWnd, IDC_BUTTON15 + i);                       //  привязка кнопок
            u_p[i] = 3;
        }
        hBut[9] = GetDlgItem(hWnd, IDC_BUTTON10);                               // подгружаем кнопку таймера
        for (int i = 0; i < 3; i++)
            ...
            if (wp == IDC_BUTTON15 + i)
            {
                ...
                // проверка на комбинации
                {
                        ...
                        u_p[2] == 0 && u_p[4] == 0 && u_p[6] == 0)      // диагональ 2
                    {
                        ...
                        if (res == IDYES)
                        {
                            t2 = t = 0;                                // время устанавливается по нулям
                            si_cross++;
                            for (int i = 0; i < 9; i++)
                            {
    ...
    case WM_TIMER:
        ...
        if (t < 10)
            wsprintfW(str, TEXT("Победа ноликов = %d, крестиков = %d, ничьих = %d   ТАЙМЕР: %d : 0%d"), si_zero, si_cross, si_draw, t2, t);
        else
            wsprintfW(str, TEXT("Победа ноликов = %d, крестиков = %d, ничьих = %d   ТАЙМЕР: %d : %d"), si_zero, si_cross, si_draw, t2, t);
